...
module.exports = {
  ...
  permanent: {
    ...
    get: function (mediaId, cb) {
      ...
      var url = permanentUrl + 'get_material?' + restful.toParam();
      restful.json(url, params, cb);
    },
    remove: function () {
  ...
  count: function (cb) {
    var url = permanentUrl + 'get_materialcount?' + restful.toParam();
    restful.json(url, null, cb);
  },
  list: function (type, limit, offset, cb) {
    var params = {
      type: type,
      offset: offset,
      count: limit
    };
    var url = permanentUrl + 'batchget_material?' + restful.toParam();
    restful.json(url, params, cb);
  },
